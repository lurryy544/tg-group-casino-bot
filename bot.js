...
const CREATOR_USERNAME = '@itcodee';
const AI_SYSTEM_PROMPT =
  'Ты — дружелюбный ассистент игрового Telegram-бота "Ракетка". Отвечай кратко, живо и по-русски. ' +
  'Тебя создал ' + CREATOR_USERNAME + ' (он же твой владелец, «господин»). ' +
  'Если спрашивают, кто тебя создал / твой создатель / кто тебя сделал — гордо отвечай: «Меня создал ' + CREATOR_USERNAME + '». ' +
...
const HELP_TEXT =
  '📖 КАК ИГРАТЬ\n\n' +
  'Новые игроки получают 100 ₽ на баланс.\n\n' +
  '💰 БАЛАНС\n' +
  'пиши: баланс\n' +
  'узнать свои рубли\n\n' +
  '🎁 БОНУС\n' +
  'пиши: бонус\n' +
  'раз в 24 часа от 50 до 500 ₽\n\n' +
  '💸 ПЕРЕВОД\n' +
  'ответь на сообщение игрока и напиши: перевод 100\n' +
  'отправить рубли другому игроку\n\n' +
  '💣 МИНЫ\n' +
  'пиши: мины 100\n' +
  'поле 5×5, в нём 5 мин. Каждая чистая клетка даёт +20% к ставке.\n' +
  'Открывай клетки и жми «Забрать куш», пока не наступил на мину!\n\n' +
  '🚀 КРАШ (ракетка)\n' +
  'пиши: краш 100\n' +
  'ракета летит, коэффициент растёт каждую секунду.\n' +
  'Забери куш до взрыва — иначе ставка сгорает!';
...
bot.hears(/^(help|\/help|start|\/start|помощь|команды|справка|как играть|правила)($|\s)/i, async (ctx) => {
  try {
    await ctx.reply(HELP_TEXT);
  } catch (error) {
    console.error('Ошибка в помощи:', error);
  }
});
...
async function main() {
  ...
  try {
    await bot.telegram.sendMessage(
      ADMIN_ID,
      '🚀 Казино-бот ' + username + '\nУспешно запущен! Напиши мне: помощь — чтобы увидеть команды.'
    );
  } catch (notifyError) {
